...
Const cTabla = "DESTINOS"
Const cCampoID = "DES_CODIGO"
Const cDesRegistro = "Destino"
...
Function SetMode(pMode As Integer)
    ...
    Select Case pMode
        Case 1
            ...
            Me.Caption = "Nuevo Destino..."
            txtID_LostFocus
            DesacCtrl txtID
        Case 2
            cmdAceptar.Enabled = False
            Me.Caption = "Editando Destino..."
            DesacCtrl txtID
        Case 3
            cmdAceptar.Visible = False
            Me.Caption = "Datos del Destino..."
            DesacCtrl txtID
        Case 4
            cmdAceptar.Enabled = True
            Me.Caption = "Eliminando Destino..."
            DesacCtrl txtID
    End Select
    ...
    If Validar(vMode) = True Then
        ...
        Select Case vMode
            Case 1 'nuevo
                ...
                cSQL = cSQL & "     (DES_CODIGO, DES_DESCRI) "
                cSQL = cSQL & "VALUES "
            ...
            Case 2 'editar
                ...
                cSQL = cSQL & "     DES_DESCRI = " & XS(txtDescri.Text)
                cSQL = cSQL & " WHERE DES_CODIGO  = " & XN(txtID.Text)
            
            Case 4 'eliminar
            
                cSQL = "DELETE FROM " & cTabla & " WHERE DES_CODIGO  = " & XN(txtID.Text)
...
Private Sub Form_Load()
    ...
    If vMode <> 1 Then
        If vFieldID <> "0" Then
            cSQL = "SELECT * FROM " & cTabla & "  WHERE DES_CODIGO = " & Mid(vFieldID, 2, Len(vFieldID) - 2)
            rec.Open cSQL, DBConn, adOpenStatic, adLockOptimistic
            If (rec.BOF And rec.EOF) = 0 Then
                'si encontró el registro muestro los datos
                txtID.Text = rec!DES_CODIGO
                txtDescri.Text = rec!DES_DESCRI
            Else
                Beep
